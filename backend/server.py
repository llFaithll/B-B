...
app.add_middleware(
    ...
    # Questa regex consente la comunicazione da qualunque dominio o sotto-dominio ospitato su Netlify
...
async def suggest_price(payload: PricingSuggestIn, user: dict = Depends(get_current_user)):
    ...
    if not groq_key:
        suggest = payload.base_price
        return {
            "suggested_price": suggest, "min_price": round(suggest * 0.8, 2),
            "max_price": round(suggest * 1.4, 2), "nights": nights,
            "total_suggested": round(suggest * nights, 2),
...
class TaxSettingsIn(BaseModel):
    ...
    property_name: Optional[str] = "Casa B&B"  # Nome personalizzato della struttura
    manager_name: Optional[str] = None          # Nome personalizzato del gestore
...
async def get_tax_settings(user: dict = Depends(get_current_user)):
    ...
    if not doc:
        return {
            ...
            "manager_name": user.get("name", "Gestore B&B")
        }
    return {
        "fee_per_night": doc.get("fee_per_night", 3.50),
        "max_nights": doc.get("max_nights", 10),
        "kids_under_age": doc.get("kids_under_age", 10),
        "property_name": doc.get("property_name", "Casa B&B"),
        "manager_name": doc.get("manager_name") or user.get("name", "Gestore B&B")
    }
...
async def generate_receipt_pdf(bid: str, guests_count: int = 1, kids_count: int = 0, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        if tax_settings:
            ...
            property_name = tax_settings.get("property_name", "Casa B&B").strip()
            manager_name = (tax_settings.get("manager_name") or user.get("name", "Gestore B&B")).strip()
        else:
            fee = 3.50
            max_n = 10
            property_name = "Casa B&B"
    ...
    try:
        ...
        story.append(Paragraph(f"<b>Ricevuta n°:</b> {str(booking.get('_id'))[:8].upper()} / {datetime.now().year}", normal_style))
        ...
        story.append(Paragraph(f"<b>Periodo:</b> dal {booking.get('checkin')} al {booking.get('checkout')} ({booking_nights} notti)", normal_style))
        story.append(Spacer(1, 20))
        
        data = [
            [Paragraph("<b>Descrizione Servizio</b>", normal_style), Paragraph("<b>Importo</b>", normal_style)],
            [Paragraph(f"Pernottamento breve ({booking_nights} notti, {g_count} ospiti)", normal_style), Paragraph(f"{gross_stay:.2f} €", normal_style)],
