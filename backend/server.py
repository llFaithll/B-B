...
FRONTEND_URL=os.environ.get(_x,_i).rstrip(_J)
...
frontend_url_env=os.environ.get(_x,_i).rstrip(_J)
allowed_origins=[frontend_url_env,_i]
...
def create_access_token(user_id,email):payload={_W:user_id,_G:email,_X:'access','exp':datetime.now(timezone.utc)+timedelta(minutes=1440)};return jwt.encode(payload,JWT_SECRET,algorithm=JWT_ALGORITHM)
...
def set_auth_cookies(response,access,refresh):response.set_cookie(_Y,access,httponly=_C,secure=_C,samesite=_j,max_age=86400,path=_J);response.set_cookie(_k,refresh,httponly=_C,secure=_C,samesite=_j,max_age=604800,path=_J)
...
CHANNEL_COMMISSION={_Z:.03,_m:.15,_M:.0,_n:.1}
...
async def refresh(request,response):
	token=request.cookies.get(_k)
	if not token:raise HTTPException(status_code=401,detail='No refresh token')
	try:
		payload=jwt.decode(token,JWT_SECRET,algorithms=[JWT_ALGORITHM])
		if payload.get(_X)!='refresh':raise HTTPException(status_code=401,detail=_l)
		user=await db.users.find_one({_D:ObjectId(payload[_W])})
		if not user:raise HTTPException(status_code=401,detail=_A0)
...
async def update_inventory(iid,payload,user=Depends(get_current_user)):
	doc=payload.model_dump();doc[_q]=datetime.now(timezone.utc).isoformat();r=await db.inventory.update_one({_D:ObjectId(iid),_B:user[_A]},{_c:doc})
...
async def alloggiati_export_zip(start_date,end_date,user=Depends(get_current_user)):
	...
	for b in bookings:
		...
		for(i,p)in enumerate(b.get(_AE,[])or[]):
			fpath=UPLOAD_DIR/p
			if fpath.exists():ext=fpath.suffix or _w;arc=f"foto_documenti/{safe_name}_{i+1}{ext}";photo_files.append((arc,str(fpath)))
	buf=BytesIO()
	...
	buf.seek(0);return StreamingResponse(buf,media_type='application/zip',headers={_v:f'attachment; filename="alloggiati_{start_date}_{end_date}.zip"'})
@api.get('/uploads/{filename}')
async def get_upload(filename,user=Depends(get_current_user)):
	fpath=UPLOAD_DIR/filename
	if not fpath.exists()or'..'in filename or _J in filename:raise HTTPException(404,'File non trovato')
	return FileResponse(str(fpath))
ADMIN_EMAIL_ENV=os.environ.get(_z,'admin@example.com')
...
async def public_registration(guest_first_name=Form(...),guest_last_name=Form(...),checkin=Form(...),checkout=Form(...),channel=Form(_M),document_number=Form(...),date_of_birth=Form(...),place_of_birth=Form(...),country_of_birth=Form(_K),citizenship=Form(_K),sex=Form('M'),document_type=Form(_a),document_place=Form(''),photos=File(default=[])):
	...
	for f in photos or[]:
		...
		ext=Path(f.filename).suffix.lower()or _w
		if ext not in[_w,'.jpeg','.png','.webp','.pdf','.heic']:continue
...
@api.post(_AG)
async def save_ross_settings(payload,user=Depends(get_current_user)):await db.settings.update_one({_B:user[_A],_f:_g},{_c:{**payload.model_dump(),_B:user[_A],_f:_g,_q:datetime.now(timezone.utc).isoformat()}},upsert=_C);return{_I:_C}
